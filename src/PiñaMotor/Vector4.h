...
public:
	...
	// Métodos
	double magnitudeSquared() const { return (pow(x, 2) + pow(y, 2) + pow(z, 2) + pow(r, 2)); }
	double magnitude() const { return sqrt(magnitudeSquared()); }

	Vector4<float> rotate(float angle, Vector4<float> axis);
	...
	/// </summary>
	Vector4<T> crossProduct(const Vector3<T>& other) const {
	...
	/// </summary>
	bool isZero() const { return ((x == 0) && (y == 0) && (z == 0) && (r == 0)); }

	/// <summary>
	/// Setea el vector a 0
	/// </summary>
	void clear() { x = y = z = r = 0; }

	/// <summary>
	/// Invierte el vector
	/// </summary>
	void inverse() { x = -x; y = -y; z = -z; r = -r; }

	/// <summary>
	/// Calcula el ángulo con respecto a otro vector
	/// </summary>
	/// <returns> Ángulo en radianes </returns>
	double angleRadians(const Vector4<T>& other) const {
